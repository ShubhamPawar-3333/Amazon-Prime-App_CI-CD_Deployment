...
output "jenkins_server_ssh_command" {
  value       = "ssh -i ${aws_key_pair.project_setup_key.key_name}.pem ubuntu@${aws_instance.jenkins_instance.public_ip}"
...
output "tools_server_ssh_command" {
  value       = "ssh -i ${aws_key_pair.project_setup_key.key_name}.pem ubuntu@${aws_instance.tools_instance.public_ip}"
  description = "SSH command to connect to the Jenkins server"
}